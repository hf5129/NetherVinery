...
public class NetherWineBottleRenderer implements NetherStorageTypeRenderer {
    @Override
    public void render(NetherStorageBlockEntity entity, PoseStack poseStack, MultiBufferSource multiBufferSource, NonNullList<ItemStack> nonNullList) {
    ...
    private void renderOne(NetherStorageBlockEntity entity, PoseStack matrices, MultiBufferSource vertexConsumers, NonNullList<ItemStack> nonNullList) {
        if(nonNullList.get(0).getItem() instanceof NetherDrinkBlockItem item){
            ClientUtil.renderBlock(getState(item), matrices, vertexConsumers, entity);
        }
    }

    private static BlockState getState(NetherDrinkBlockItem item){
        return item.getBlock().defaultBlockState().setValue(NetherWineBottleBlock.FAKE_MODEL, false);
    }

    private void renderTwo(NetherStorageBlockEntity entity, PoseStack matrices, MultiBufferSource vertexConsumers, NonNullList<ItemStack> nonNullList) {
        NetherDrinkBlockItem item1 = nonNullList.get(0).getItem() instanceof NetherDrinkBlockItem item ? item : null;
        NetherDrinkBlockItem item2 = nonNullList.get(1).getItem() instanceof NetherDrinkBlockItem item ? item : null;
    ...
    private void renderThree(NetherStorageBlockEntity entity, PoseStack matrices, MultiBufferSource vertexConsumers, NonNullList<ItemStack> nonNullList) {
        NetherDrinkBlockItem item1 = nonNullList.get(0).getItem() instanceof NetherDrinkBlockItem item ? item : null;
        NetherDrinkBlockItem item2 = nonNullList.get(1).getItem() instanceof NetherDrinkBlockItem item ? item : null;
        NetherDrinkBlockItem item3 = nonNullList.get(1).getItem() instanceof NetherDrinkBlockItem item ? item : null;
